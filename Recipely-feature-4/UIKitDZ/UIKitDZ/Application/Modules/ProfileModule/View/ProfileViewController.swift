...
//        navigationItem.title = Local.profileModule
...
extension ProfileViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        switch cellType {
        case .userInfo, .bonuses, .privacy, .partners, .logout:
            return 1
        default:
            return 0
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .partners:
            ...
            cell.configureCell(info: safePresenter.loadInfo().partners)
            cell.selectionStyle = .none
            cell.accessoryType = .disclosureIndicator
            return cell
        case .logout:
...
extension ProfileViewController: ProfileViewControllerProtocol {
    func showNameChangeAlert() {
        let alert = UIAlertController(
            title: Local.ProfileModule.usernameAlertTitle,
            message: nil,
            preferredStyle: .alert
        )
        alert.addTextField { textField in
            textField.placeholder = Local.ProfileModule.usernameDefaultText
            let cancelAction = UIAlertAction(
                title: Local.ProfileModule.canAction,
                style: .cancel
            )
            let okAction = UIAlertAction(
                title: Local.ProfileModule.okAction,
                style: .default
            ) { [weak self] _ in
    ...
    func showLogoutAlert() {
        let alert = UIAlertController(
            title: Local.ProfileModule.logoutAlertTitle,
            message: nil,
            preferredStyle: .alert
        )
        let cancelAction = UIAlertAction(
