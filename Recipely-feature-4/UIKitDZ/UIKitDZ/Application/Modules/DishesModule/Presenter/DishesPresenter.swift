...
//        view?.updateState(sender: sender)
    ...
    func updateDishes() {
        ...
        dishType = mainCourses.contains { $0 == dishType }
            ? "Main course"
            : dishType
        let coreDataSavedDishes = coreDataService?.fetchDishes(by: dishType)
        if (coreDataSavedDishes?.isEmpty) != nil {
            print("CHTO TO")
            networkService?.getDishes(
                ...
                completionHandler: { result in
                    ...
                    case let .success(dishes):
                        ...
                        self.coreDataService?.create(dishes: dishes, by: dishType)
                        self.dishes = dishes
